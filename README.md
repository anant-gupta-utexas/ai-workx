...
> **Note:** The consult-experts skill has moved to the DEV-ESSENTIALS plugin. The chief-of-staff skill previously lived in its own plugin; it was folded into essentials in v2.4.0. The `pressure-test` skill was added in v2.6.0. In v2.7.0, the `maintaining-wiki` skill's ingest gained open-problem extraction (top-3 by buildability → `docs/00_ops/inbox/inbox.md`) and its lint gained state-cadence write-back (`last_wiki_lint` + `last_wiki_lint_score` → `docs/00_ops/meta/state.md`).
